 ...
 **/
const Stealth: React.FC<ModalProps> = ({
  show,
  onClose,
  _ENS_,
  chain,
  ...
  } = useSendTransaction(sendPayment);
  ...
  function statementIPNSKey(extradata: string, type: string) {
    let _toSign = `Requesting Signature To Generate IPNS Key\n\nOrigin: ${
      ["recordhash", "storage"].includes(type) ? ENS : origin
    }\nKey Type: ed25519\nExtradata: ${extradata}\nSigned By: ${caip10}`;
    let _digest = _toSign;
    return _digest;
  }
    ...
    try {
      ...
      if (_response?.address) {
        ...
        if (_response.address === ccip2Contract) {
          ...
          const _stealth = await refreshRecord(
            ["text", "stealth"],
            _response,
            _ENS,
            false
          ...
          setStealth(_stealth || "");
          const _RSA = await refreshRecord(
            ["text", "rsa"],
            _response,
            _ENS,
            false
    ...
    try {
      if (_record[0] === "text") {
        ...
        if (_response) {
          ...
          } else if (_record[1] === "rsa") setRSA(_response);
          if (_trigger) {
  ...
  React.useEffect(() => {
    ...
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [_ManagerLegacy_, _OwnerLegacy_, _OwnerWrapped_]);
  ...
  React.useEffect(() => {
    if (sendSuccess && txSuccess) {
      const pin = async () => {
        ...
        setTimeout(() => {
          setSuccess(
            '<span>Payment <span style="color: lightgreen">Sent Successfully</span></span>'
          );
          ...
          message: statementIPNSKey(
            ethers.utils.keccak256(
              ethers.utils.solidityPack(
                ...
                [
                  ethers.utils.keccak256(
                    ethers.utils.solidityPack(["string"], [_modalData])
                  ),
                  _Wallet_,
                ]
              )
          ...
          await _resolver
            ...
            .then((response) => {
              if (!response) {
                ...
                _encryptionResult = cryptico.encrypt(
                  `{"payer":"${_Payer}","payee":"${_Payee}","amount":"${_Amount}"}`,
                  _RSA
                );
                ...
                if (!priorState.includes("stealth") && newValues["stealth"]) {
                  ...
                  priorState.includes("stealth") &&
                  !newValues["stealth"]
                ) {
                  setStates((prevState) =>
                    prevState.filter((item) => item !== "stealth")
                  );
                }
              }
            })
                  ...
                  const keygen = async () => {
                    ...
                    const pin = async () => {
                      if (data.response.ipfs && w3name && w3nam3 && gas) {
                        ...
                        setSuccess(
                          trigger === "stealth"
                            ? '<span><span style="color: lightgreen">Stealth Record Set</span>! You may now <span style="color: cyan">Receive</span> Private Payments</span>'
                            : '<span><span style="color: lightgreen">Encryption Key Set</span>! You may now <span style="color: cyan">Send</span> Private Payments</span>'
                        );
                        setSuccessModal(true);
                        setPayeeAddr("");
                        doEnjoy();
                              ...
                              ["stealth", "rsa"].includes(item.type) && (
                                <button
                                  ...
                                  onClick={() => {
                                    ...
                                    setHelp(
                                      item.type === "stealth"
                                        ? '<span>Please set the <span style="color: cyan">Stealth Invoice</span> to <span style="color: orange">Receive</span> Private Payment<span>'
                                        : RSA
                                        ? '<span>Encryption Key <span style="color: lime">Set</span><span>'
                                        : `<span>Please Set <span style="color: cyan">Encryption Key</span> To <span style="color: orange">Send</span> Private Payments<span>`
                                    );
                                  }}
                                  data-tooltip={
                                    item.type === "stealth"
                                      ? "Set Encrypted Invoice"
                                      : RSA
                                      ? "Encryption Key Set"
                            ...
                            }
                            {
                              ...
                              ["stealth", "rsa"].includes(item.type) &&
                                ...
                                history.ownerstamp.length > 0 && (
                                  <button
                                    className={
                                      !["", ".", "0", "1"].includes(refresh) &&
                                      [item.type].includes(refresh)
                                        ? "button-tiny blink"
                                        : "button-tiny"
                                    }
                                    onClick={() => {
