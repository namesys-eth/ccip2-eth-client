 ...
 **/
const Preview: React.FC<ModalProps> = ({
  show,
  onClose,
  _ENS_,
  chain,
  handleParentModalData,
  handleParentTrigger,
}) => {
  // React States
  ...
  const {
    isSuccess: txSuccess1of3,
    isError: txError1of3,
    isLoading: txLoading1of3,
  } = useWaitForTransaction({
    hash: response1of3?.hash,
  });
  const {
    isSuccess: txSuccess2of3,
    isError: txError2of3,
    isLoading: txLoading2of3,
  } = useWaitForTransaction({
    hash: response2of3?.hash,
  });
  const {
    isSuccess: txSuccess3of3,
    isError: txError3of3,
    isLoading: txLoading3of3,
  } = useWaitForTransaction({
  ...
  function statementIPNSKey(extradata: string, type: string) {
    let _toSign = `Requesting Signature To Generate IPNS Key\n\nOrigin: ${
      ["recordhash", "gateway"].includes(type) ? ENS : origin
    }\nKey Type: ed25519\nExtradata: ${extradata}\nSigned By: ${caip10}`;
    let _digest = _toSign;
    return _digest;
  }
  ...
  function statementRecords(
    ...
    let _signer =
      "eip155:" + chain + ":" + ethers.utils.computeAddress(`0x${signer}`);
  ...
  function statementManager(signer: string) {
    let _signer =
      "eip155:" + chain + ":" + ethers.utils.computeAddress(`0x${signer}`); // Convert secp256k1 pubkey to ETH address
  ...
  function genExtradata(key: string, _recordValue: string) {
    ...
    const _extradata = ethers.utils.hexlify(
      ethers.utils.solidityPack(["bytes"], [toPack])
    );
    return _extradata;
  }
  ...
  async function signManager() {
    ...
    if (keypairSigner) {
      const SIGN_APPROVE = async () => {
        signMessage({
          message: statementManager(keypairSigner[0]),
        });
      };
      SIGN_APPROVE();
    }
        ...
        message: statementSignerKey(
          ethers.utils.keccak256(
            ethers.utils.solidityPack(
              ...
              [
                ethers.utils.keccak256(
                  ethers.utils.solidityPack(["string"], [_value])
                ),
                _Wallet_,
              ]
            )
    ...
    const getGasAmountForContractCall = async () => {
      ...
      if (key === "contenthash") {
        gasAmount = await contract.methods
          .setContenthash(
            ethers.utils.namehash(ENS),
            ensContent.encodeContenthash(value).encoded
          )
        ...
        gasAmount = await contract.methods
          .setText(ethers.utils.namehash(ENS), key, value)
          .estimateGas({ from: _Wallet_ });
        ...
        gasAmount = await contract.methods
          .setAddr(
            ethers.utils.namehash(ENS),
            _type,
            `0x${formatsByName[key.toUpperCase()]
              .decoder(value)
              .toString("hex")}`
          )
          .estimateGas({ from: _Wallet_ });
        ...
        gasAmount = await contract.methods
          .setAddr(ethers.utils.namehash(ENS), value)
          .estimateGas({ from: _Wallet_ });
      }
      return gasAmount;
    };
      ...
      if (_response?.address) {
        ...
        if (_response.address === ccip2Contract) {
          ...
          if (_history.ownerstamp.length >= 1) {
            if (
              _IPFS._sequence &&
              _history.timestamp.revision &&
              Number(_IPFS._sequence) ===
                Number(_history.timestamp.revision) - 1 &&
              _Storage[1]
            ) {
              _history.addr ? setAddr(_history.addr) : setAddr("");
              _history.contenthash
              ...
              _history["com.twitter"]
                ? setTwitter(_history["com.twitter"])
                : setTwitter("");
              _history["com.discord"]
                ? setTwitter(_history["com.discord"])
                : setDiscord("");
              _history["xyz.farcaster"]
                ? setFarcaster(_history["xyz.farcaster"])
                : setFarcaster("");
          ...
          setSync(true);
          const _addr = await refreshRecord(
            ["addr", ""],
            _response,
            _ENS,
            false
          ...
          setAddr(_addr || "");
          const _contenthash = await refreshRecord(
            ["contenthash", ""],
            _response,
            _ENS,
            false
          ...
          setAvatar(_avatar || "");
          const _email = await refreshRecord(
            ["text", "email"],
            _response,
            _ENS,
            false
          ...
          setGithub(_github || "");
          const _url = await refreshRecord(
            ["text", "url"],
            _response,
            _ENS,
            false
          ...
          setUrl(_url || "");
          const _twitter = await refreshRecord(
            ["text", "com.twitter"],
            _response,
            _ENS,
            false
          ...
          setTwitter(_twitter || "");
          const _discord = await refreshRecord(
            ["text", "com.discord"],
            _response,
            _ENS,
            false
          ...
          setDiscord(_discord || "");
          const _farcaster = await refreshRecord(
            ["text", "xyz.farcaster"],
            _response,
            _ENS,
            false
          ...
          const _nostr = await refreshRecord(
            ["text", "nostr"],
            _response,
            _ENS,
            false
          ...
          setNostr(_nostr || "");
          const _btc = await refreshRecord(
            ["address", "btc"],
            _response,
            _ENS,
            false
          ...
          setBTC(_btc || "");
          const _ltc = await refreshRecord(
            ["address", "ltc"],
            _response,
            _ENS,
            false
          ...
          setLTC(_ltc || "");
          const _doge = await refreshRecord(
            ["address", "doge"],
            _response,
            _ENS,
            false
          ...
          setDOGE(_doge || "");
          const _sol = await refreshRecord(
            ["address", "sol"],
            _response,
            _ENS,
            false
          ...
          setSOL(_sol || "");
          const _atom = await refreshRecord(
            ["address", "atom"],
            _response,
            _ENS,
            false
  ...
  async function writeRevision(
    revision: Name.Revision | undefined,
    gas: {},
    timestamp: string,
    _ipfs: string
  ) {
      ...
      await fetch(`${SERVER}:${PORT}/read`, {
        ...
        .then((data) => {
          let _HISTORY = {
            ...
            "com.github": data.response["com.github"],
            url: data.response.url,
            "com.twitter": data.response["com.twitter"],
            "com.discord": data.response["com.discord"],
            "xyz.farcaster": data.response["xyz.farcaster"],
  ...
  React.useEffect(() => {
    if (safeTrigger === "1") {
      if (trigger && !write) {
        if (trigger === "storage") {
          setHashType(
            confirmModalState.modalData === "0" ? "recordhash" : "gateway"
          );
        } else {
          if (optionsModalState.trigger) {
            setHashType(
              optionsModalState.modalData === "1"
                ? "recordhash"
                : optionsModalState.modalData === "2"
                ? "gateway"
                : "ownerhash"
            );
            setProcessCount(1);
  ...
  React.useEffect(() => {
    if (
      gatewayModalState.trigger &&
      gatewayModalState.modalData !== undefined &&
      write
    ) {
      setLoading(true);
  ...
  React.useEffect(() => {
    if (
      ...
      saltModalState.modalData !== undefined &&
      !keypairIPNS &&
      trigger &&
      safeTrigger
    ) {
      ...
      if (hashType !== "gateway") {
        const CIDGen = async () => {
          ...
          if (write) {
            if (
              CID_IPNS &&
              (CID_IPNS === _Recordhash || CID_IPNS === _Ownerhash)
            ) {
              setGoodSalt(true);
              setCID(CID_IPNS);
            } else if (
              CID_IPNS &&
              CID_IPNS !== _Recordhash &&
              CID_IPNS !== _Ownerhash
            ) {
              setSaltModalState({
                modalData: undefined,
  ...
  React.useEffect(() => {
    if (conclude) {
      getUpdate(
        ...
        recordhash && recordhash.startsWith("ipns://")
          ? "recordhash"
          : ownerhash && ownerhash.startsWith("ipns://")
          ? "ownerhash"
          : "gateway",
        recordhash && recordhash.startsWith("ipns://")
          ? "recordhash"
                  ...
                  const keygen = async () => {
                    ...
                    const pin = async () => {
                      if (data.response.ipfs && w3name && w3nam3 && gas) {
                        ...
                        const toPublish =
                          "/ipfs/" + data.response.ipfs.split("ipfs://")[1];
                        // @W3Name broadcast
                        ...
                        if (!history.revision) {
                          ...
                          let _revision_ = Revision.decode(
                            new Uint8Array(
                              Object.values(
                                JSON.parse(JSON.stringify(history.revision))
                              )
                            )
                          );
  ...
  React.useEffect(() => {
    if (isMigrateSuccess && txSuccess1of3 && migrated && !write && !crash) {
      if (states.includes("resolver")) {
        setSuccess(
          optionsModalState.modalData === "1"
            ? '<span><span style="color: lightgreen">Resolver Migrated</span>! You may now set <span style="color: cyan">IPNS Storage</span> next</span>'
            : '<span><span style="color: lightgreen">Resolver Migrated</span>! You may now set <span style="color: cyan">HTTP Storage</span> next</span>'
        );
        setSuccessModal(true);
        doEnjoy();
      } else {
        setSuccess(
          '<span style="color: lightgreen">Off-chain Setup Complete with <span style="color: cyan">Ownerhash</span>. Enjoy!</span>'
        );
        setSuccessModal(true);
        doEnjoy();
      }
  ...
  React.useEffect(() => {
    if (recordhash && txSuccess2of3 && !write && !crash) {
      setSuccess(
        '<span style="color: lightgreen">Off-chain Setup Complete with <span style="color: cyan">Recordhash</span>. Enjoy!</span>'
      );
      setSuccessModal(true);
      doEnjoy();
    } else if (recordhash && txSuccess3of3 && !write && !crash) {
      setSuccess(
        '<span style="color: lightgreen">Off-chain Setup Complete with <span style="color: cyan">HTTP Gateway</span>. Enjoy!</span>'
      );
      setSuccessModal(true);
      doEnjoy();
    }
              ...
              >
                ...
                >
                  <button
                    ...
                    data-tooltip={
                      migrated || resolver === ccip2Contract
                          ...
                          >
                                  ...
                                  <span>
                                    ...
                                    >
                                      {item.type === "storage"
                                        ...
                                        : item.type === "contenthash"
                                        ? "public"
                                        : item.type === "com.github"
                                        ? "code"
                                        : item.type === "url"
                                        ? "share"
                                        : item.type === "email"
                                        ...
                                        ? "groups"
                                        : item.type === "com.discord"
                                        ? "group_add"
                                        : item.type === "xyz.farcaster"
                                        ? "people_alt"
                                        : item.type === "nostr"
                                        ? "groups"
                                        : item.type === "btc"
                                  ...
                                  ownerhash && (
                                    <button
                                      ...
                                      onClick={() => {
                                        ...
                                        setHelp(
                                          item.type === "resolver"
                                            ? '<span>Resolver is <span style="color: lime">Migrated</span></span>'
                                            : `<span>Global <span style="color: cyan">${
                                                ownerhash.startsWith("https://")
                                                  ? ownerhash ===
                                                    ...
                                                    : "Custom Gateway"
                                                  : "IPNS"
                                              }</span> is set as <span style="color: cyan">Ownerhash</span></span>`
                                        );
                                      }}
                                      data-tooltip={
                                        item.type === "resolver"
                                            ...
                                            : `<span><span style="color: cyan">${
                                                recordhash.startsWith(
                                                  ...
                                                  ? recordhash ===
                                                    ...
                                                    : "Custom Gateway"
                                                  : "IPNS"
                                              }</span> is set as Recordhash<span>`
                                        );
                                      }}
                                      data-tooltip={
                                        item.type === "resolver"
                                  ...
                                  !ownerhash && (
                                    <button
                                      ...
                                      onClick={() => {
                                        ...
                                        setHelp(
                                          item.type === "resolver"
                                            ? '<span>Resolver is <span style="color: orange">not Migrated</span><span>'
                                            : '<span>Resolver is <span style="color: orange">not Migrated</span><span>'
                                        );
                                      }}
                                      data-tooltip={
                                        item.type === "storage"
                                          ? "Resolver Not Migrated"
                                          : "Resolver Not Migrated"
                                      }
                                    >
                                      <div
                                  ...
                                  (recordhash || ownerhash) && (
                                    <button
                                      ...
                                      onClick={() => {
                                        ...
                                        setHelp(
                                          item.type === "resolver"
                                            ? '<span>Resolver <span style="color: orange">not Migrated</span></span>'
                                            : recordhash
                                            ? `<span><span style="color: cyan">${
                                                recordhash.startsWith(
                                                  "https://"
                                                  ...
                                                  ? "Gateway"
                                                  : "Recordhash"
                                              }</span> <span style="color: lime">is Set as Recordhash</span></span>`
                                            : `<span><span style="color: cyan">${
                                                ownerhash.startsWith("https://")
                                                  ? "Gateway"
                                                  : "Ownerhash"
                                              }</span> <span style="color: lime">is Set as Ownerhash</span></span>`
                                        );
                                      }}
                                      data-tooltip={
                                        recordhash
                                  ...
                                  (recordhash || ownerhash) && (
                                    <button
                                      ...
                                      onClick={() => {
                                        ...
                                        setHelp(
                                          queue < 0
                                            ? '<span><span style="color: orange">Too Soon To Update</span>. Please wait at least <span style="color: cyan">one hour</span> between updates</span>'
                                            : '<span><span style="color: lime">Ready</span> For Next Record Update</span>'
                                        );
                                      }}
                                      data-tooltip={
                                        queue < 0
                                ...
                                !C.config.includes(item.type) &&
                                  ...
                                  history.ownerstamp.length > 0 && (
                                    <button
                                      ...
                                      data-tooltip={
                                        ![item.type, ".", "0", "1"].includes(
                                          refresh
                                        )
                                          ? item.value === history[item.type]
                                            ? `Record in Sync with ${
                                                hashType === "gateway"
                                                  ? "Gateway"
                                                  : "IPNS"
                                              }`
                                            : "Record not in Sync. Click to refresh"
                                          : ![".", "", "0", "1"].includes(
                                              refresh
                                            )
                                          ? "Refresh in Progress"
                            ...
                            >
                              ...
                              >
                                ...
                                <span className="material-icons-round smoller">
                                  {multiEdit(item)
                                    ? legit[item.type]
                                      ? "task_alt"
                                      : "cancel"
