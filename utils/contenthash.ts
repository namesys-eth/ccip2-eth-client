...
export type ContentHashProvider =
  | "ipfs"
  | "swarm"
  | "onion"
  | "skynet"
...
export const decodeContenthash = (
  encodedObjOrString?: EncodedContentHash | string
) => {
  let decoded;
  let protocolType;
  let error;
  ...
  let encoded =
    typeof encodedObjOrString === "string"
      ? encodedObjOrString
      : encodedObjOrString?.encoded;

  if (typeof encodedObjOrString === "object") {
    if (encodedObjOrString.error)
      return { protocolType: null, decoded: encodedObjOrString.error };
...
export const encodeContentId = (
  ...
  try {
    if (protocolType === "ipfs" && contentId?.length >= 4)
      ...
      encoded = `0x${contentHash.encode("ipns-ns", contentId)}`;
...
const contentHashProtocolToProviderMap: {
  [key: string]: ContentHashProvider | undefined;
} = {
  ipfs: "ipfs",
  ipns: "ipfs",
  bzz: "swarm",
