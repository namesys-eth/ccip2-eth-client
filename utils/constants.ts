...
// Config records
export const config = ["resolver", "storage"];
// Uneditable records in Preview modal
export const forbidden = ["resolver"];
// Blocked records in Preview modal
export const blocked = ["pubkey", "ltc", "doge", "sol", "atom"];
...
export const signedRecord =
  "function signedRecord(address recordSigner, bytes memory recordSignature, bytes memory approvedSignature, bytes memory result)";
export const signedRedirect =
  "function signedRedirect(address recordSigner, bytes memory recordSignature, bytes memory approvedSignature, bytes memory redirect)";
...
export function encodeContenthash(contenthash: string) {
  if (contenthash && !contenthash.startsWith("https://")) {
    const ensContentHash = ensContent.encodeContenthash(
      `ipns://${contenthash}`
    );
...
export function isAvatar(value: string) {
  return (
    ...
    (value.startsWith("ipfs://") &&
      (ipfsRegexCID0.test(value) || ipfsRegexCID1.test(value))) ||
...
export async function getIPFSHashFromIPNS(
  ipnsKey: string,
  cacheBuster: Number
) {
  try {
...
export async function getABI(contractAddress: string): Promise<any | null> {
  try {
    ...
    }.etherscan.io/api?module=contract&action=getabi&address=${contractAddress}&apikey=${
